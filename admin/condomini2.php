...
$where = " AND t.number_route = '$id' ";
$value = "";
$counter = 0;
$rel = "";
$group = ' GROUP BY number_estate ';
...
if( isset( $_REQUEST['c'] ) && ctype_alnum( str_replace( array( '.', '_'),'',$_REQUEST['c']) ) ) { $column = strtolower( $_REQUEST['c'] ); }  else { $column = "number_estate"; } 
if( isset( $_REQUEST['o'] ) && strtolower( $_REQUEST['o'] ) == 'desc' ){ $order = "desc"; } else { $order = "asc"; }

$query  = "SELECT t.*,COUNT(*) as myCounter FROM condomini AS t $rel WHERE 1 " . $where . " $group ORDER BY " . $column . " " . $order . " ";
...
if( $rows_sel == 0 ) { $value = "<tr><td colspan='20'><br><b> &nbsp; Nessun risultato trovato</b><br><br></td></tr>"; }
	...
	while( $line = mysqli_fetch_array( $result ) ) {
		...
		$value .= "<td><b><a href='letture.php?strut11=$line[number_route]&cond11=$line[number_estate]'>" . $line['myCounter'] . " x</a></b></td>\n";
		$value .= "</tr>\n";
	}
}
...
<td><?php echo column( "myCounter", "desc", 'CONTATORI', $column, $order ) ?></td>
</tr>
<?php echo $value; ?>
</table>
</div>
